...
const MAX_AGENTS = 30;
const APP_VERSION = 'v28.0-FailSafe';
...
function zeniteSystem() {
    return {
        ...
        // ============================================================
        // 1. INICIALIZAÇÃO BLINDADA (FAIL-SAFE)
        // ============================================================
        async initSystem() {
            console.log(`ZENITE ${APP_VERSION} INIT`);
            
            // Cão de guarda: Se tudo falhar, libera a tela em 4s
            setTimeout(() => { if(this.systemLoading) this.systemLoading = false; }, 4000);

            try {
                // 1. Verifica hash de erro (Retorno de OAuth cancelado)
                ...
                if (isGuestStore) {
                    ...
                    // Se não tiver sessão, o código segue e o finally desliga o loader, mostrando o login
                }

            } catch (e) {
                console.error("Init Error:", e);
            } finally {
                // 4. OBRIGATÓRIO: Libera a interface
            ...
            supabase.auth.onAuthStateChange(async (event, session) => {
                if (event === 'SIGNED_IN' && session) {
                    this.systemLoading = true; // Breve load enquanto busca dados
                    this.user = session.user;
                    this.isGuest = false;
            ...
            setInterval(() => {
                if (this.currentView === 'sheet' && !this.isGuest) this.notify('✨ Lembrete: Tudo salvo na nuvem?', 'info');
            }, 600000);
        },
        ...
        async doSocialAuth(provider) {
            ...
            // Não ativamos systemLoading aqui para não bloquear se o popup falhar
