 ...
 * Version: v66.0-Rescue-Mission
 * Changelog:
 * - Fix Critical: Código totalmente expandido (sem one-liners) para eliminar erros de sintaxe.
 * - Restore: Recuperação da lógica de inicialização do Banco de Dados e Mouse.
 * - Stability: Lógica de 'Revert' e 'Dice Tray' mantida e legível.
 */
...
const RPG_CORE = {
    ...
    calculateDerived(className, levelStr, attrs) {
        ...
        // Helper seguro para pegar atributo
...
// Variáveis Globais de Performance (Fora do Alpine para não pesar a reatividade)
let cursorX = -100;
let cursorY = -100;
...
function zeniteSystem() {
    return {
        ...
        async initSystem() {
            ...
            setTimeout(() => { 
                if(this.systemLoading) {
                    console.warn("Forcing System Load...");
        ...
        // --------------------------------------------------------------------
        // [SECTION] DICE TRAY & REVERT SYSTEM
        // --------------------------------------------------------------------
        toggleDiceTray() {
            ...
            if(local) {
                try {
                    ...
                    Object.keys(parsed).forEach(k => { 
                        // Recupera chars mesmo com estrutura antiga
        ...
        processImport(e) { 
            ...
            r.onload = (evt) => { 
                try { 
                    ...
                    this.notify('Erro arquivo.', 'error'); 
                } 
            }; 
            r.readAsText(f); 
        }
