/**
 * ZENITE OS - Main Controller v82.0
 * Fix Critical: Mouse Engine Initialization Order & Fullscreen Logic
 */
...
// --- MOUSE ENGINE (CORRIGIDO) ---
// Agora espera o DOM estar pronto antes de buscar o elemento
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Verifica se settings existe antes de acessar
        const settings = window.Alpine && window.Alpine.store('zenite') ? window.Alpine.store('zenite').settings : { mouseTrail: true };
        
        if (settings.mouseTrail) {
            isCursorHover = !!e.target.closest('button, a, input, select, .cursor-pointer, .draggable-handle');
        }
    });
...
function zeniteSystem() {
    return {
        ...
        settings: {
            ...
            crtMode: false, // OFF por padrão para não atrapalhar debug
            sfxEnabled: true,
            themeColor: 'cyan'
        },
        ...
        async initSystem() {
            ...
            try {
                ...
                this.setupListeners();
                // setupCursorEngine removido daqui pois agora roda globalmente no DOMContentLoaded
                this.setupWatchers();
                ...
                this.notify("Erro crítico: " + (err.message || "Falha"), "error"); 
        ...
        finishWizard() {
            ...
            if(typeof RPG === 'undefined') return this.notify("RPG Core off", "error");
        ...
        openWizard() { if(this.agentCount >= CONFIG.MAX_AGENTS) return this.notify('Limite atingido.', 'error'); this.wizardStep = 1; this.wizardPoints = 8; this.wizardData = { class: '', name: '', identity: '', age: '', history: '', photo: null, attrs: {for:-1, agi:-1, int:-1, von:-1, pod:-1} }; this.wizardOpen = true; },
        ...
        exportIdentityCard() {
            ...
            html2canvas(element, { backgroundColor: '#050507', scale: 2, useCORS: true, ignoreElements: (el) => el.classList.contains('no-print') }).then(canvas => { const link = document.createElement('a'); link.download = `ID_${this.char.name}.png`; link.href = canvas.toDataURL('image/png'); link.click(); if(typeof SFX !== 'undefined') SFX.play('save'); this.notify("ID Card exportado!", "success"); });
        },
        ...
        setupListeners() {
            ...
            document.addEventListener('mouseover', (e) => {
                ...
                if (target && target !== lastHovered) { if(typeof SFX !== 'undefined') SFX.play('hover'); lastHovered = target; } else if (!target) { lastHovered = null; }
            });
        },
        ...
        handleLogoClick() {
            ...
            if (this.logoClickCount >= 5) {
                ...
                return; // Para aqui, não tenta fullscreen
            }
            
            this.logoClickTimer = setTimeout(() => { this.logoClickCount = 0; }, 2000);
            
            // Toggle Fullscreen (se não for o 5º clique)
        ...
        setupCursorEngine() { /* Dummy for compatibility, real logic is global above */ }
    };
}