 ...
 * Version: v65.0-Stable-Expanded
 * Fixes:
 * - Código expandido (sem one-liners) para evitar erros de sintaxe.
 * - Lógica de inicialização blindada.
 * - Recuperação do acesso aos dados locais e nuvem.
 */
...
function zeniteSystem() {
    return {
        ...
        // --- INIT & EVENTS ---
        async initSystem() {
            // Safety Timeout para remover loader se travar
            setTimeout(() => { 
                if(this.systemLoading) this.systemLoading = false; 
            }, 5000);
            ...
            if(local) {
                try {
                    ...
                    Object.keys(parsed).forEach(k => { 
                        // Verificação flexível para recuperar chars antigos
