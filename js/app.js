...
// SAFE INITIALIZATION: Previne tela branca se a biblioteca não carregar
...
const MAX_AGENTS = 30;
const APP_VERSION = 'v46.0-Bulletproof';
...
function zeniteSystem() {
    return {
        ...
        async initSystem() {
            // 1. FAILSAFE PRIORITÁRIO: Garante que o loader suma em 4s independente de erro
            setTimeout(() => { 
                if(this.systemLoading) {
                    this.systemLoading = false;
                    if(!supabase) this.notify("Erro Crítico: Rede ou Banco de Dados offline.", "error");
                }
            }, 4000);
            ...
            // Verificação crítica de dependência
            if (!supabase) {
                this.log("CRITICAL: Supabase library not loaded via CDN.", "error");
                // Força modo guest para não travar
                this.isGuest = true;
            ...
            if (window.matchMedia("(pointer: fine)").matches && trail) {
                document.addEventListener('mousemove', (e) => { 
                    mouseX = e.clientX; mouseY = e.clientY; 
                    const target = e.target;
                    // Check defensivo para evitar erro de nulo
            ...
            // Lógica de Inicialização Normal
