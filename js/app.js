 ...
 * Version: v87-modular-prep
 * Structure: Monolithic file prepared for splitting
 */
...
// --- MODULE: CORE SYSTEM (Alpine Component) ---
function zeniteSystem() {
    return {
        ...
        // --- STATE: CONFIGS ---
        settings: {
            mouseTrail: true, compactMode: false, 
        ...
        handleLogoClick() {
            ...
            // Reset rápido (400ms) para exigir cliques rápidos
        ...
        setupCursorEngine() {
            ...
            const renderLoop = () => {
                if (!trail) return;
                const isAuthenticated = this.user || this.isGuest;
                if (isAuthenticated && this.settings.mouseTrail && !this.isMobile) {
        ...
        loadLocal(key) {
            ...
            if(local) {
                try {
                    ...
                    const validChars = {};
                    Object.keys(parsed).forEach(k => { if(!['config','trayPos','hasSeenTip'].includes(k) && parsed[k]?.id) { validChars[k] = this.sanitizeChar(parsed[k]); } });
        ...
        async performRevert() {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if(this.activeCharId && this.chars[this.activeCharId]) { this.char = this.sanitizeChar(this.chars[this.activeCharId]); } else { this.currentView = 'dashboard'; this.char = null; }
        ...
        async fetchCloud() {
            ...
            try {
                ...
                if (data && data.data) {
                    ...
                    Object.keys(cloudData).forEach(k => { if(!['config','hasSeenTip'].includes(k)) { merged[k] = this.sanitizeChar(cloudData[k]); } });
