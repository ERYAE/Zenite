/**
 * ZENITE OS - Core Application v2.3 (Full Fixed)
 * Integrado com NetLink, SheetModule e GMPanel.
 * Contém todas as funções de sistema, nuvem e UI.
 */
    ...
    return {
        ...
        setupWatchers() {
            ...
            this.$watch('currentView', (val) => { 
                if (val !== 'sheet') { this.diceTrayOpen = false; this.revertConfirmMode = false; } 
            });
        ...
        async fetchCloud() {
            ...
            try {
                ...
                if (data && data.data) {
                    ...
                    // Merge de Personagens (Local + Cloud)
                    let merged = { ...this.chars }; 
                    let hasLocalOnly = false;
                    
                    Object.keys(cloudData).forEach(k => { 
                        if(!['config','hasSeenTip'].includes(k)) merged[k] = cloudData[k]; 
                    });
                    
                    // Preserva chars locais que não estão na nuvem e marca para sync
        ...
        // ============================
        // LÓGICA DE FICHA (Via SheetModule)
        // ============================
        modAttr(key, val) {
        ...
        recalcDerivedStats() {
            if(this.sheetLogic && this.char) {
                ...
                if(window.UTILS) this.updateRadarChart();
            }
        },
        ...
        // ============================
        openWizard() { 
            if(this.agentCount >= (window.CONFIG ? window.CONFIG.MAX_AGENTS_LOCAL : 30)) return this.notify("Capacidade Máxima!", "error");
            this.wizardStep = 1; 
            this.wizardPoints = 8; 
        ...
        async createCampaignUI() { 
            const n = prompt("Nome da Campanha:"); 
            if(n && this.netLink) { 
                await this.netLink.createCampaign(n); 
                this.campaigns = this.netLink.campaigns; 
        ...
        async joinCampaignUI() { 
            ...
            if(c && this.netLink) { 
                await this.netLink.joinCampaign(c); 
                this.campaigns = this.netLink.campaigns; 
                this.notify("Acesso Concedido!", "success"); 
            } 
        },
        ...
        askLogout() { this.askConfirm('SAIR?', 'Dados locais serão limpos.', 'warn', () => { localStorage.removeItem('zenite_cached_db'); this.supabase.auth.signOut(); window.location.reload(); }); },
        ...
        applyCrop() { 
            const r=window.UTILS.getCroppedImage(this.cropperInstance); 
            if(r){ if(this.uploadContext==='wizard')this.wizardData.photo=r; else this.char.photo=r; } 
        ...
        // Settings
        toggleSetting(k,v) { 
            if(v) { this.settings[k]=v; if(k==='themeColor') this.applyTheme(v); } 
            else { 
                this.settings[k]=!this.settings[k]; 
                if(k==='sfxEnabled' && window.SFX) window.SFX.toggle(this.settings[k]); 
                if(k==='crtMode') this.updateVisualState(); 
            } 
            this.saveLocal(); 
            if(this.user) this.syncCloud(true);
        },
        ...
        processImport(e) { 
            window.UTILS.readJSON(e.target.files[0], (d, err) => { 
                if(err) return this.notify('Erro arquivo.', 'error'); 
        ...
        performRevert() { 
            this.loadLocal(this.isGuest ? 'zenite_guest_db' : 'zenite_cached_db'); 
        ...
        handleLogoClick() { 
            this.logoClickCount++; 
            if(this.logoClickCount >= 5){ this.triggerSystemFailure(); } 
        ...
        triggerSystemFailure() {
            ...
            setTimeout(() => { this.systemFailure = false; }, 5000);
        },
        handleKeys(e) { 
            /* Konami Code Logic se necessário */ 
        }
    };
}